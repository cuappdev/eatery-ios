...
function enumerationDeclaration(generator, type) {
    ...
    graphql_2.sortEnumValues(values).forEach((value, i) => {
        if (!value.description || value.description.indexOf("\n") === -1) {
            generator.printOnNewline(`  "${value.value}"${i === nValues - 1 ? ";" : " |"}${printing_1.wrap(" // ", value.description || undefined)}`);
        }
        else {
...
function structDeclarationForInputObjectType(generator, type) {
    ...
    language_1.typeDeclaration(generator, {
        ...
        const properties = propertiesFromFields(generator.context, Object.values(type.getFields()).map(v => {
            return Object.assign({ fieldName: v.name, responseName: v.name }, v, { description: v.description || undefined });
        }));
...
function getPossibleTypeNames(generator, property) {
    return generator.context.schema
        .getPossibleTypes(graphql_1.getNamedType((property.fieldType ||
        property.type)))
        .map(type => type.name);
}
